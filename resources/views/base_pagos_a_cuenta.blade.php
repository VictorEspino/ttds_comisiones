<x-app-layout>
    ...
    <div class="flex flex-col w-full bg-white text-gray-700 shadow-lg rounded-lg">
        <div class="w-full rounded-t-lg bg-ttds-encabezado p-3 flex flex-col border-b border-gray-800"> <!--ENCABEZADO-->
            <div class="w-full text-xl font-bold text-gray-100">Pagos a cuenta de comisiones</div>
        </div> <!--FIN ENCABEZADO-->
        @if(session('status')!='')
        ...
        <div class="w-full rounded-b-lg bg-ttds-secundario p-3 pb-7 flex flex-col"> <!--CONTENIDO-->
            <div class="w-full flex flex-col lg:flex-row justify-between space-y-3 lg:space-y-0">
                ...
                <div class="w-full lg:w-1/2 flex flex-col">
                    <form action="{{route('base_pagos_a_cuenta')}}" class="" method="GET">
            ...
            <div class="flex flex-col lg:flex-row lg:space-x-5 flex items-start justify-center pt-2">
                <div id="tabla" class="w-full pt-5 flex flex-col"> <!--TABLA DE CONTENIDO-->
                    <div class="w-full flex justify-center pb-3"><span class="font-semibold text-sm text-gray-700">Pagos a cuenta de comisiones</span></div>
                        ...
                        <div class="table">
                            <div class="table-row-group flex">
                                <div class="table-row rounded-tl-lg rounded-tr-lg">
                                    ...
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm"><center>Total pago</center></div>
                                    ...
                                    @endif
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2  {{(Auth::user()->perfil=='distribuidor')?'rounded-tr-lg':''}} text-sm"><center>Estatus Actual</center></div>
                                    @if(Auth::user()->perfil!="distribuidor")
                                ...
                                <div class="table-row">
                                    ...
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm"><center>${{number_format($pago->cantidad,0)}}</center></div>
                                    ...
                                    @if(Auth::user()->perfil=="distribuidor")
                                        ...
                                        @else
                                            <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm"><center>{!!$pago->user->detalles->emite_factura=="1"?'<a href="/facturar_pago_a_cuenta/'.$pago->id.'">Tramitar</a>':'NA'!!}</div>
                                        @endif
                                    @endif
                                    ...
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm">
                                        <form method="POST" action="{{route('cambiar_estatus_pago_a_cuenta')}}">
                                            @csrf
                                            <input type="hidden" name="id" value="{{$pago->id}}">
                                            <input type="hidden" name="nombre" value="{{$pago->user->name}}">
                                            <input type="hidden" name="monto" value="{{$pago->cantidad}}">
                            ...
                            <div class="table-row-group flex">
                                ...
                                <div class="table-row">
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm">
                                        ...
                                        <b>${{number_format($pago->cantidad,0)}}</b><br>
                                    ...
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm">
                                        <form method="POST" action="{{route('cambiar_estatus_pago_a_cuenta')}}">
                                            ...
                                            <input type="hidden" name="id" value="{{$pago->id}}">
                                            <input type="hidden" name="nombre" value="{{$pago->user->name}}">
                                            <input type="hidden" name="monto" value="{{$pago->pago_a_cuenta}}">
                                            <input type="hidden" name="nuevo_estatus" value="{{$pago->aplicado?'0':'1'}}">
                                            <button class="rounded border border-gray-500 bg-gray-100 text-sm text-gray-700 font-normal py-1 px-3">{{$pago->aplicado?'Pendiente':'Procesado'}}</button>
                                        </form>
                                    </div>
