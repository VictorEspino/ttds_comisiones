        ...
        <div class="w-full rounded-b-lg bg-ttds-secundario p-3 pb-7 flex flex-col"> <!--CONTENIDO-->
            <div class="w-full flex flex-col lg:flex-row justify-between space-y-3 lg:space-y-0">
                <div class="w-full lg:w-1/2 flex flex-col">
                    <form action="{{route('pagos')}}" class="" method="GET">
                        ...
                        <div class="w-full flex flex-col md:flex-row p-1">
                            ...
                            <div class="w-full md:w-5/6">
                                <select name="calculo" class="w-full md:w-2/3 rounded p-1 border border-gray-300">
                                    <option value=""></option>
                                    @foreach($calculos as $calculo)
                                    <option value="{{$calculo->id}}" {{$calculo_id==$calculo->id?'selected':''}}>{{$calculo->descripcion}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="table">
                            <div class="table-row-group flex">
                                <div class="table-row rounded-tl-lg rounded-tr-lg">
                                    ...
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm"><center>Distribuidor</center></div>
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm"><center>Tipo</center></div>
                                ...
                                <div class="table-row">
                                    <div class="table-cell px-2 border-l border-b border-gray-300 font-ligth text-ttds {{$color?'bg-gray-100':'bg-white'}}">
                                        <center><a href="/estado_cuenta_distribuidor/{{$pago->calculo_id}}/{{$pago->user_id}}/{{$pago->version}}"><i class="fas fa-balance-scale"></i></center></a>
                                    </div>
                                    ...
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm">{{$pago->version=="1"?"Adelanto":"Cierre"}}</div>
                                    <div class="table-cell border-l border-b border-gray-300 font-ligth {{$color?'bg-gray-100':'bg-white'}} text-gray-700 py-1 px-2 mx-2 text-sm">{{$pago->calculo->descripcion}}</div>
                    ...
                    <div class="md:hidden w-full flex justify-center px-2">
                        <div class="table w-10/12">
                            <div class="table-row-group flex">
                                <div class="table-row rounded-tl-lg rounded-tr-lg">
                                    <div class="table-cell font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm rounded-tl-lg"></div>
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm {{Auth::user()->perfil=="distribuidor"?'rounded-tr-lg':''}}"><center>Datos</center></div>
                                    @if(Auth::user()->perfil!="distribuidor")
                                    <div class="table-cell border-l font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm rounded-tr-lg"><center>Cambiar<br>estatus a:</center></div>
                                    @endif
                                </div>
