<x-app-layout>
    <x-slot name="header">
            {{ __('Pagos a cuenta de comisiones') }}
    </x-slot>

    <div class="flex flex-col w-full bg-white text-gray-700 shadow-lg rounded-lg">
        <div class="w-full rounded-t-lg bg-ttds-encabezado p-3 flex flex-col border-b border-gray-800"> <!--ENCABEZADO-->
            <div class="w-full text-lg font-semibold text-gray-100">Distribuidores - Pagos a cuenta</div>            
            ...
            <div class="flex flex-col lg:flex-row lg:space-x-5 flex items-start justify-center pt-2">
                        ...
                        <div class="table">
                            <div class="table-row-group flex">
                                <div class="table-row rounded-tl-lg rounded-tr-lg">
                                    ...
                                    <div class="table-cell font-semibold bg-ttds-encabezado text-gray-200 py-1 px-2 mx-2 text-sm rounded-tr-lg">Registrar pago a cuenta</div>
                ...
                <div id=forma class="hidden w-full lg:w-1/3 flex justify-center pt-8 flex-col"> <!--FORMA DETALLES-->
                    ...
                    <div class="w-full flex flex-col border border-gray-300 bg-white rounded-b">
                        <form action="{{route('pagos_a_cuenta')}}" method="POST">
                            @csrf
                            ...
                            <div class="w-full text-xs text-ttds p-2">
                                Se refiere al periodo de ventas del cual se aplica el pago a cuenta
                            </div>
                            ...
                            <div class="w-full px-2 flex flex-row">
                                <div class="w-full">
                                    <span class="text-xs text-ttds">Cantidad</span><br>
                                    <input class="w-full rounded p-1 border border-gray-300" type="text" value="{{old('cantidad')}}" id="cantidad" name="cantidad">
                                    @error('anticipo')
                                        <br><span class="text-xs italic text-red-700 text-xs">{{ $message }}</span>
                                    @enderror
                                </div>
    ...
    <script>
        ...
        function toogleForma(user_id,nombre)
        {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if(this.responseText!='')
                    {
                        ...
                        cadena="Pagos a cuenta previos por cobrar<br><div class='table'><div class='table-row'><div class='table-cell bg-ttds px-3'></div><div class='table-cell bg-ttds text-gray-200 px-3'>Periodo</div><div class='table-cell bg-ttds text-gray-200 px-3'>Cantidad</div><div class='table-cell bg-ttds text-gray-200 px-3'>Descripcion</div></div>";
                        respuesta.forEach(element => cadena=cadena+"<div class='table-row'>"+"<div class='table-cell text-red-700 font-bold px-3'><a href='javascript:BorrarPago("+element['id']+","+element['cantidad']+","+user_id+",\""+nombre+"\")'><i class='fas fa-trash'></i></a></div><div class='table-cell px-3'>"+element['periodo']['descripcion']+"</div><div class='table-cell px-3'>"+formatter.format(element['cantidad'])+"</div><div class='table-cell px-3'>"+element['descripcion']+"</div></div>");
            ...
            xmlhttp.open("GET", "/pagos_a_cuenta_consulta/" + user_id, true);
            xmlhttp.send();
        }
        function BorrarPago(id,cantidad,user_id,nombre)
        {
            if(confirm("Esta seguro que desea borrar este pago a cuenta de "+cantidad+" para "+nombre))
            {
                var xmlhttp = new XMLHttpRequest();
                xmlhttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                ...
                xmlhttp.open("GET", "/pagos_a_cuenta_borrar/" + id, true);
                xmlhttp.send();
            }
        }
    <?php
